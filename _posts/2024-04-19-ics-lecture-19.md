...
최소 `size` 바이트의 **정렬(Alignment)**[^alignment]된 메모리 블록을 할당한다. 성공 시 해당 메모리 블록을 가리키는 포인터를 반환하고, `size == 0`이거나 오류 발생 시 null 포인터를 반환한다.
...
할당된 블록에서 데이터를 저장하기 위해 애플리케이션이 요청한 메모리를 **페이로드(Payload)**라 하며, 할당자의 구현 방식에 따라 추가로 사용되는 메모리를 **오버헤드(Overhead)**라 한다. 상황을 단순화하기 위해 할당자가 힙의 크기를 줄이지 않고 늘리기만 한다고 가정하면, 최대 메모리 사용률을 다음과 같이 나타낼 수 있다.

- $P_k$ : 총 페이로드 (현재 할당된 모든 블록의 페이로드 합)
- $H_k$ : 현재 힙 크기
- $U_k$ : $k$번째 요청 후의 최대 메모리 사용률
...
- **Implicit free list**: 별도의 리스트 없이, 헤더(블록 크기)를 통해 힙의 모든 블록을 순회하면서 free 블록을 식별한다. 간단하지만, 효율성이 떨어질 수 있다.
...
- **Blocks sorted by size**: 트리를 사용하여, free 블록을 크기 순으로 정렬한다.
