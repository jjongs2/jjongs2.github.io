...
- 공유되는 데이터 구조에 접근할 때는 일시적으로 모든 시그널을 차단해야 한다.
...
4. 더이상 보류 중인 시그널이 없으므로, 부모 프로세스는 메인 루프에서 스핀(spin)하게 된다.
...
- `main()`과 `handler()`는 concurrent flow로서 각각 공유 데이터 구조인 작업 큐에 접근하고 있으며, 작업 큐를 업데이트하는 동안 모든 시그널을 차단하고 있다.
...
int main(int argc, char *argv[]) {
    ...
    while (1) {
        ...
        while (!pid)
            Sigsuspend(&prev);
        Sigprocmask(SIG_SETMASK, &prev, NULL);  /* Opnblock SIGCHLD */
        /* Do some work after receiving SIGCHLD */
        printf(".");
    }
    printf("\n");
