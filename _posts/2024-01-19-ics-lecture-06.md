...
- `for` 구문의 초기 조건식은 참인 경우가 많기 때문에, `-O1` 옵션으로 컴파일하면 위의 주석 처리한 부분을 무시할 수 있다.
...
- `case`의 최솟값이 0이 아닐 경우, 컴파일러는 바이어스(bias)를 추가하여 최솟값이 0이 되도록 조정한다.
