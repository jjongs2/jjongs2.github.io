...
> 일치하는 태그가 없거나, 유효 비트가 0이면 **캐시 부적중**이 발생한다.
{: .prompt-warning }
...
- 유효 비트가 0이므로 **캐시 부적중**이 발생하여, 메모리에서 데이터 블록을 가져온다.
...
- 일치하는 태그가 없으므로 **캐시 부적중**이 발생하여, 메모리에서 데이터 블록을 가져와 기존의 캐시 블록과 교체한다.
...
- **Miss rate**: 캐시 부적중이 발생하는 비율
- **Hit time**: 캐시 적중 여부를 판단하고, 적중 시 데이터를 읽어오는 데 소요되는 시간
- **Miss penalty**: 캐시 부적중이 발생했을 때, 메모리에서 데이터를 가져오기 위해 추가로 소요되는 시간
...
- 중첩 루프에서는 안쪽 루프에 주목하여 캐시 부적중을 최소화해야 한다.

- 반복적으로 참조되는 변수, 특히 지역 변수는 레지스터에 저장될 가능성이 높아 캐시 친화적이다.

- 배열을 순차적으로 참조(stride-1)하면 캐시 블록이 효율적으로 사용되어 캐시 적중률이 높아진다.
...
- stride가 증가함에 따라 공간 지역성이 감소한다.
  - stride-8이면 폭이 64바이트인 것인데, 이는 블록 크기와 같다. 즉 매번 다른 블록을 참조하게 되므로, 공간 지역성이 없는 셈이다. 이러한 이유로 stride가 8 이상일 때 그래프가 평평한 것을 볼 수 있다.
...
- `a`에 대한 행 방향 참조는 4번마다 1번씩 캐시 부적중이 발생한다. (stride-1 참조 패턴)
- `b`에 대한 열 방향 참조는 매번 캐시 부적중이 발생한다.

따라서 반복당 평균 캐시 부적중 횟수는 1.25이다. `jik` 순으로 배치한 루프도 이와 동일하다.
...
- `b`와 `c`에 대한 행 방향 참조는 각각 4번마다 1번씩 캐시 부적중이 발생한다.

따라서 반복당 평균 캐시 부적중 횟수는 0.5이다. `ikj` 순으로 배치한 루프도 이와 동일하다.
...
- `a`와 `c`에 대한 열 방향 참조는 각각 매번 캐시 부적중이 발생할 것이다.

따라서 반복당 평균 캐시 부적중 횟수는 2이다. `kji` 순으로 배치한 루프도 이와 동일하다.
...
- `a`에 대한 행 단위 참조는 $\frac{n}{8}$번의 캐시 부적중이 발생한다.
- `b`에 대한 열 단위 참조는 $n$번의 캐시 부적중이 발생한다.

따라서 `c`의 요소당 캐시 부적중 횟수는 $\frac{9}{8} n$이고, $n^2$ 개의 요소가 있으므로 총 캐시 부적중 횟수는 $\frac{9}{8} n^3$이다.
...
- `a`와 `b`에 대한 블록 단위 참조는 각각 $\frac{B^2}{8}$번의 캐시 부적중이 발생한다.
- `a`와 `b`에 대한 블록으로 된 행/열 단위 참조는 각각 $\frac{B^2}{8} \times \frac{n}{B} = \frac{Bn}{8}$번의 캐시 부적중이 발생한다.

따라서 `c`의 블록당 캐시 부적중 횟수는 $\frac{Bn}{4}$이고, $(\frac{n}{B})^2$개의 블록이 있으므로 총 캐시 부적중 횟수는 $\frac{1}{4B} n^3$이다. $B$ 값이 클수록 캐시 부적중 횟수가 감소하므로, $3B^2 < C$를 만족하는 가장 큰 정수 값을 선택하는 것이 최선이다.
...
[^cache-miss]: ["Lecture 11: The Memory Hierarchy." [Online].]({{ site.url }}/posts/ics-lecture-11/#general-cache-concepts)
